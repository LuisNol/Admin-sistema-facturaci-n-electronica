...
import { UNITS, type Product, type ProductCategorie, type ProductConfigResponse, type ProductResponse } from "@/types/products";
...
const route = useRoute("products.edit");
...
const store = async () => {
    ...
    try {
        const resp: AxiosResponse<ProductResponse> = await HttpClient.post(
                    "products/"+product_selected.value?.id,formData);
...
const show = async() => {
    try {
        ...
        if(product_selected.value){
            ...
            is_isc.value = product_selected.value.percentage_isc > 0 ? 2 : 1; 
